...
double Client::obtenirTotalAPayer() const
{
	// TODO : À modifier
    double montant = 0;
	for (pair<int, Produit*> pairProduit : panier_->obtenirConteneur()) 
	{
		montant += pairProduit.second->obtenirPrix();
	}
	
	for (panier_->obtenirConteneur()::iterator it = panier_->obtenirConteneur().begin(); it < panier_->obtenirConteneur().end(); it++)
	{

	}

    for (unsigned int i = 0; i < panier_.size(); i++)
        montant += panier_->obtenirConteneur().obtenirPrix();
    return montant;
}
